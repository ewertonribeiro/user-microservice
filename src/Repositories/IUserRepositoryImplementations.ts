...
} from './Interface/IUserRespository';
...
export class UserRepository implements IUserRepositoryInterface {
  ...
  async createUser({
    ...
    await db.query<IUser>(
      `INSERT INTO users (name,lastname,email,password,token,id) VALUES('${newUser.name}','${newUser.lastname}','${newUser.email}','${newUser.password}','${newUser.token}','${newUser.id}');`,
    );
  ...
  async findUserByEmail(email: string): Promise<IUser | undefined> {
    const query = await db.query<IUser>(
  ...
  async createSession(id: string): Promise<IUser> {
    ...
    const updated = await db.query<IUser>(
      `SELECT * FROM users WHERE id = '${id}';`,
    );
